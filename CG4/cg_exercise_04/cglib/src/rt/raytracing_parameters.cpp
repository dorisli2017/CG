...
"BVH Traversal Time:      Time spent on bvh traversal for primary hit\n"
	...
	{
		redraw |= ImGui::Combo("Texture Filter", &tex_filter_mode, &tex_filter_mode_names[0], TEXTURE_FILTER_MODE_COUNT);
		redraw |= ImGui::Combo("Texture Wrap", &tex_wrap_mode, &tex_wrap_mode_names[0], TEXTURE_WRAP_MODE_COUNT);
	}
