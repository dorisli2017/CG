	...
	int axis)
{
	...
	cg_assert(num_triangles > MAX_TRIANGLES_IN_LEAF);
...
build_bvh(int node_idx, int first_triangle_idx, int num_triangles, int depth)
{
	...
	if(node.num_triangles <= MAX_TRIANGLES_IN_LEAF){
        ...
        node.aabb.extend(kindMax);
        //test
		cg_assert(nodes[node.left].num_triangles+nodes[node.right].num_triangles==node.num_triangles);
	}
}
...
intersect_recursive(const Ray &ray, int idx, float *nearest_intersection, Intersection* isect) const
{
	...
	// This is an inner node. Recurse into child nodes.
		...
		bool hit = false;
		float t_min_left = -FLT_MAX;
		float t_max_left = FLT_MAX;
		float t_min_right=-FLT_MAX;
		...
		if(min_kind !=-1){
			...
			if(max_kind != -1){
				if(max_min <=*nearest_intersection){
